...
def _build_multipart(fields: dict[str, str], file_path: Path) -> tuple[bytes, str]:
    ...
    boundary = f"----ZTAWatchBoundary{uuid.uuid4().hex}"
    eol = b"\r\n"
    buf = io.BytesIO()
...
def _post_whisper(endpoint: str, api_key: str, model: str, audio_path: Path) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "zta-watch/1.0 (+blacksheepsystems; python-urllib)",
    }
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        try:
            ...
            if attempt < MAX_ATTEMPTS - 1:
                print(
                    f"[zta-watch] whisper HTTP {exc.code} — retrying in {delay:.1f}s "
            ...
            if attempt < MAX_ATTEMPTS - 1:
                ...
                print(
                    f"[zta-watch] whisper network error ({type(exc).__name__}: {exc}) — "
...
def transcribe_video(
    ...
    print(f"[zta-watch] extracting audio for Whisper ({backend})…", file=sys.stderr)
    audio_path = extract_audio(video_path, audio_out)
    size_kb = audio_path.stat().st_size / 1024
    print(f"[zta-watch] audio: {size_kb:.0f} kB — uploading to {backend} Whisper…", file=sys.stderr)
    ...
    print(f"[zta-watch] transcribed {len(segments)} segments via {backend}", file=sys.stderr)
